...
/* function is defined but never used. Commenting the following to fix/avoid linting errors */
...
function determineData(type, i, hash) {
  ...
  h["backgroundColor"] = colorGenerator(
    i,
    "bg",
    type,
    hash["y_axis_values" + i].length
  );
  h["borderColor"] = colorGenerator(
    i,
    "bo",
    type,
    hash["y_axis_values" + i].length
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    for (var j = 0; j < completeData[i].length; j++) {
      if (counter >= maxval) {
        ...
        completeData[i][j] != null ||
        completeData[i][j] != undefined
      ) {
...
function determineHeaders(mat) {
  ...
  for (var i = 0; i < mat[0].length; i++) {
    if (i == 0) {
      ...
      if (
        typeof mat[0][i] == typeof mat[0][i - 1] &&
        typeof mat[0][i] != "object"
      ) {
        headers[i] = mat[0][i];
