...
class View {
  ...
  plotGraph(hash, length, type, flag, library) {
    if (library == 'chartjs') {
      this.chartjsPlotter = new ChartjsPlotter(
        hash,
        length,
        type,
        flag,
      ...
      this.plotlyjsPlotter = new PlotlyjsPlotter(
        hash,
        length,
        type,
        flag,
  ...
  afterSampleData(flag, type) {
    document.getElementById(this.plotGraphId).onclick = e => {
      ...
      $('input:checkbox[name=' + this.tableYInputName + ']:checked').each(
        (index, element) => {
          columns.push(element.value);
        }
      );
      for (var i = 0; i < columns.length; i++) {
        hash['y_axis_values' + i] = this.csvParser.completeCsvMatrix[
          columns[i]
        ];
  ...
  tableGenerator(
    ...
    for (let i = 0; i < this.csvParser.csvSampleData[0].length; i++) {
      var tr = document.createElement('tr');
      for (let j = 0; j < this.csvParser.csvHeaders.length; j++) {
        let td = document.createElement('td');
        td.appendChild(
          document.createTextNode(this.csvParser.csvSampleData[j][i])
        );
        tr.appendChild(td);
      }
  ...
  listenersForIntegration() {
    $('#' + this.fileUploadId).change(e => {
      ...
      document.getElementById('popover' + this.fileUploadId).style.display =
        'inline';
      document.getElementById(
        'popover' + this.csvStringUploadId
      ).style.display = 'none';
      document.getElementById(
        'popover' + this.googleSheetUploadId
      ).style.display = 'none';
      document.getElementById(
        'popover' + this.remoteFileUploadId
      ).style.display = 'none';
    ...
    $('#' + this.csvStringUploadId).change(() => {
      ...
      this.handleFileSelectstring(
        document.getElementById(this.csvStringUploadId).value
      );
    });
    $('#' + this.googleSheetUploadId).change(() => {
      ...
      sheetLink = document.getElementById(this.googleSheetUploadId).value;
      var sheetURL =
    ...
    $('#' + this.remoteFileUploadId).change(() => {
      ...
      this.sendRemoteFileToHandler(
        document.getElementById(this.remoteFileUploadId).value
      );
    });
  }
  ...
  drawHTMLView() {
    this.element.innerHTML =
      ...
      '> Create Spreadsheet<i class="fa fa-plus" aria-hidden="true"></i></button> <button type="button" class="btn btn-info codapExport" id=' +
