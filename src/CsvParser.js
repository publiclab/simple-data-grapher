...
class CsvParser {
    ...
    for (let i = 0; i < this.csvHeaders.length; i++) {
      ...
      for (var j = 0; j < this.completeCsvMatrix[i].length; j++) {
        if (counter >= maxval) {
          ...
          this.completeCsvMatrix[i][j] !== null ||
          this.completeCsvMatrix[i][j] !== undefined
        ) {
  ...
  matrixForCompleteData() {
    ...
    for (let i = this.csvFileStart; i < this.csvMatrix.length; i++) {
  ...
  completeMatrixForGoogleSheet() {
    ...
    for (let i = 0; i < this.csvHeaders.length; i++) {
      this.csvHeaders[i] = this.csvHeaders[i].slice(
        4,
        this.csvHeaders[i].length
      );
    }
  ...
  determineHeaders() {
    ...
    for (var i = 0; i < this.csvMatrix[0].length; i++) {
      if (i == 0) {
        ...
        if (
          (typeof this.csvMatrix[0][i] == typeof this.csvMatrix[0][i - 1] &&
            typeof this.csvMatrix[0][i] != "object") ||
          (typeof this.csvMatrix[0][i] != typeof this.csvMatrix[0][i - 1] &&
            csvHeadersLocal[i - 1].substring(0, 6) == "Column")
        ) {
          csvHeadersLocal[i] = this.csvMatrix[0][i];
        }
    ...
    if (flag && csvHeadersLocal.length != this.csvMatrix[0].length) {
      this.csvFileStart = 0;
      for (let i = 0; i < this.csvMatrix[0].length; i++) {
        csvHeadersLocal[i] = "Column" + (i + 1);
      }
    }
  ...
  createTranspose() {
    ...
    for (let i = 0; i < this.completeCsvMatrix.length; i++) {
      completeCsvMatrixTransposeLocal[0][i] = this.csvHeaders[i];
    }
    for (let i = 0; i < this.completeCsvMatrix.length; i++) {
      for (var j = 0; j < this.completeCsvMatrix[0].length; j++) {
        completeCsvMatrixTransposeLocal[j + 1][i] = this.completeCsvMatrix[i][
          j
        ];
      }
    }
