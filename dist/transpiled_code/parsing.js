...
function plotGraph(hash, length, type, ci, flag) {
  ...
  $('.carousel').carousel(2); // saveAsImage();
...
function determineHeaders(mat) {
  ...
  for (var i = 0; i < mat[0].length; i++) {
    if (i == 0) {
      ...
      if (_typeof(mat[0][i]) == _typeof(mat[0][i - 1]) && _typeof(mat[0][i]) != 'object') {
        headers[i] = mat[0][i];
      } else if (_typeof(mat[0][i]) == 'object') {
...
function parse(file) {
  ...
  Papa.parse(file, {
    ...
    complete: function complete() {
      console.log(mat);
      $('.carousel').carousel(1); //calling a function to determine headers for columns
