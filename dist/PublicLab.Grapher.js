(function () {
...
})()(
	{
						...
						_createClass(ChartjsPlotter, [
							{
								...
								value: function determineType() {
									if (
										...
										this.graphType == 'Stepped' ||
										this.graphType == 'Point'
									) {
										return 'line';
									} else if (
										this.graphType == 'Horizontal'
									) {
								...
								value: function determineData(i) {
									...
									h['backgroundColor'] =
										this.colorGenerator(
											i,
											'bg',
											this.dataHash['y_axis_values' + i]
												.length
										);
									h['borderColor'] = this.colorGenerator(
										i,
										'bo',
										this.dataHash['y_axis_values' + i]
											.length
							...
							},
							{
								...
								value: function saveAsImageFunction(
									canvId
								) {
							...
							},
							{
								...
								value: function createSaveAsImageButton(
									canvasDiv,
									canvasId
								) {
									var saveImageButton =
							...
							},
							{
								...
								value: function plotGraph() {
									...
									document
										.getElementById(this.canvasContainerId)
										.appendChild(div);
									var ctx = canv.getContext('2d');
									var configuration =
										this.determineConfig();
									new Chart(ctx, configuration);
									this.createSaveAsImageButton(
										div,
										canv.id
		...
		2: [
						...
						_createClass(CsvParser, [
							{
								...
								value: function callbackForLocalFile(
									...
									this.completeCsvMatrixTranspose =
										this.createTranspose();
									this.codapHeaders =
										this.headersForCodap();
									this.codapMatrix =
										this.completeMatrixForCodap();
							...
							},
							{
								...
								value: function allFunctionHandler(
									functionParameter
								) {
									if (functionParameter == 'local') {
										...
										if (
											functionParameter == 'csvstring' ||
											functionParameter == 'remote'
										) {
											this.csvFile =
												this.csvFile.split('\n');
							...
							},
							{
								...
								value: function startFileProcessing() {
									...
									if (
										...
										SimpleDataGrapher.SimpleDataGrapher.elementIdSimpleDataGraphInstanceMap[
											self.elementId
										].view.continueViewManipulation(self);
									}
							...
							},
							{
								...
								value: function extractSampleData() {
									...
									if (
										this.completeCsvMatrix.length[0] < 5
									) {
										maxval =
											this.completeCsvMatrix[0].length;
									}
									...
									for (
										...
										for (
											var j = 0;
											j < this.completeCsvMatrix[i].length;
											j++
										) {
											if (counter >= maxval) {
												break;
											} else if (
												this.completeCsvMatrix[i][j] !==
													null ||
												this.completeCsvMatrix[i][j] !==
													undefined
											) {
												if (
													typeof this.completeCsvMatrix[i][
														j
													] === 'number'
												) {
													bool = true;
							...
							},
							{
								...
								value: function matrixForCompleteData() {
									...
									for (
										var i = this.csvFileStart;
										i < this.csvMatrix.length;
										i++
									) {
										for (
											var j = 0;
											j < this.csvHeaders.length;
											j++
										) {
											completeCsvMatrixLocal[j].push(
												this.csvMatrix[i][j]
											);
										}
									}
							...
							},
							{
								...
								value:
									function completeMatrixForGoogleSheet() {
										var matrixComplete = [];
										...
										for (
											...
											this.csvHeaders[i] = this.csvHeaders[
												i
											].slice(4, this.csvHeaders[i].length);
										}
							...
							},
							{
								...
								value: function completeMatrixForCodap() {
									...
									for (
										var i = 1;
										i <
										this.completeCsvMatrixTranspose.length;
										i++
									) {
										var element = {};
							...
							},
							{
								...
								value: function determineHeaders() {
									...
									for (
										var i = 0;
										i < this.csvMatrix[0].length;
										i++
									) {
										if (i == 0) {
											if (
												typeof this.csvMatrix[0][i] ==
												'string'
											) {
												csvHeadersLocal[i] =
													this.csvMatrix[0][i];
											...
											if (
												(_typeof(this.csvMatrix[0][i]) ==
													_typeof(
														this.csvMatrix[0][i - 1]
													) &&
													_typeof(this.csvMatrix[0][i]) !=
														'object') ||
												(_typeof(this.csvMatrix[0][i]) !=
									...
									if (
										flag &&
										csvHeadersLocal.length !=
											this.csvMatrix[0].length
									) {
										this.csvFileStart = 0;
							...
							},
							{
								...
								value: function createTranspose() {
									...
									for (
										var i = 0;
										i <= this.completeCsvMatrix[0].length;
										i++
									) {
									...
									for (
										var i = 0;
										i < this.completeCsvMatrix.length;
										i++
									) {
										completeCsvMatrixTransposeLocal[0][i] =
											this.csvHeaders[i];
									}
									...
									for (
										...
										for (
											var j = 0;
											j < this.completeCsvMatrix[0].length;
											j++
										) {
											completeCsvMatrixTransposeLocal[
												j + 1
											][i] = this.completeCsvMatrix[i][j];
										}
									}
			...
			{ './SimpleDataGrapher': 4, papaparse: 11 },
		],
		3: [
			function (require, module, exports) {
				'use strict';
						...
						_createClass(PlotlyjsPlotter, [
							{
								...
								value: function determineType2() {
									if (
										this.graphType == 'Horizontal' ||
										this.graphType == 'Vertical'
									) {
										return 'bar';
									} else if (
										this.graphType == 'Pie' ||
										this.graphType == 'Doughnut' ||
										this.graphType == 'Radar'
									) {
										return 'pie';
									} else if (
										this.graphType == 'Basic' ||
		...
		4: [
			function (require, module, exports) {
				...
				var SimpleDataGrapher = function SimpleDataGrapher(
					elementId
				) {
						...
						_createClass(View, [
							{
								...
								value: function handleFileSelectlocal(
									event
								) {
									this.csvFile = event.target.files[0];

									if (
										this.csvFile['name'].split('.')[1] !=
										'csv'
									) {
										alert('Invalid file type');
									} else {
										$('#' + this.dragDropHeadingId).text(
											this.csvFile['name']
										);
										var self = this;
							...
							},
							{
								...
								value: function handleFileSelectstring(
									val
								) {
							...
							},
							{
								...
								value:
									function usingPreviouslyUploadedFile() {
										var self = this;
										self.csvParser = new CsvParser(
											'dummy',
											self.elementId,
							...
							},
							{
								...
								value: function handleFileSelectGoogleSheet(
									googleSheetData
								) {
							...
							},
							{
								...
								value: function getValueGoogleSheet(
									googleSheetLink
								) {
									var self = this;
									$.getJSON(
										googleSheetLink,
										function (data) {
											self.handleFileSelectGoogleSheet(
												data.feed.entry
											);
										}
									);
							...
							},
							{
								...
								value: function sendRemoteFileToHandler(
									val
								) {
									var _this = this;
									...
									fetch(proxyurl + url)
										...
										.then(function (contents) {
											return _this.handleFileSelectremote(
												contents
											);
										})
										['catch'](function (e) {
							...
							},
							{
								...
								value: function handleFileSelectremote(
									remoteVal
								) {
									...
									if (library == 'chartjs') {
										this.chartjsPlotter =
											new ChartjsPlotter(
												...
												type,
												flag,
												this.canvasContinerId,
												this.elementId,
												this.graphCounting
											);
									} else {
										this.plotlyjsPlotter =
											new PlotlyjsPlotter(
												hash,
												length,
												type,
							...
							},
							{
								...
								value: function setTooltip(importType) {
									if (
										importType === 'container_drag_drop'
									) {
										return 'Select a local file from your system';
									} else if (
										importType === 'container_csv_string'
									) {
										var x =
											'Type in or Paste a CSV string. \r\n';
										x += 'Example: \r\n';
										x += 'A,B,C \r\n';
										x += '1,2,3';
										return x;
									} else if (
										importType === 'container_remote_link'
									) {
										return 'Type in or Paste the link of a remote CSV file. Example: \
            http://example.com/example.csv';
									} else if (
										importType === 'container_google_sheet'
									) {
							...
							},
							{
								...
								value: function createPopover(buttonId) {
									var self = this;
									var html =
										'<div id="myForm" class="hide"><label for="title" class="popover_headings">File Title:</label><input type="text" name="title" id=' +
										'title' +
										buttonId +
										' class="form-control input-md"><label for="desc" class="popover_headings">File Description:</label><textarea rows="3" name="desc" id=' +
										'desc' +
										buttonId +
										' class="form-control input-md"></textarea><button type="button" class="btn btn-primary popover_headings" id="save"> Save</button></div>';
									$('#' + buttonId)
										.popover({
											placement: 'bottom',
							...
							},
							{
								...
								value: function createButtons(
									...
									if (userLoginCheck == 'yes') {
										...
										save_file_button.classList.add(
											'btn-primary'
										);
										save_file_button.innerHTML = 'Save CSV';
										save_file_button.id =
											this.elementId + '_save_CSV';
										var upload_prev_file =
											document.createElement('button');
										upload_prev_file.classList.add('btn');
										upload_prev_file.classList.add(
											'btn-primary'
										);
										upload_prev_file.innerHTML =
											'Choose a previously uploaded file';
										upload_prev_file.id =
											this.elementId + '_prev_file';
										var publish_research_button =
											document.createElement('button');
										publish_research_button.classList.add(
											'btn'
										);
										publish_research_button.classList.add(
											'btn-primary'
										);
										publish_research_button.innerHTML =
											'Publish as a Research Note';
										publish_research_button.id =
											this.elementId + '_publish';
										var container = document.getElementById(
											this.upload_button_container
										);
										var div_container =
											document.createElement('div');
										div_container.appendChild(
											save_file_button
										);
										div_container.appendChild(
											upload_prev_file
										...
										var container2 =
											document.getElementById(
												this.feature_button_container
											);
										container2.appendChild(
											publish_research_button
										);
							...
							},
							{
								...
								value: function createDataset() {
									...
									hashCollections['attrs'] =
										this.csvParser.codapHeaders;
									...
									dataset2['resource'] =
										'dataContext[my dataset].item';
									dataset2['values'] =
										this.csvParser.codapMatrix;
									var dataset3 = {};
							...
							},
							{
								...
								value: function codapExport() {
									...
									var modal_body = document.getElementById(
										'body_for_CODAP'
									);
									modal_body.innerHTML = iframeBody;
									var iframe =
										document.getElementById('codap-iframe');
									...
									var codapIframe =
										document.getElementById('codap-iframe');
									var rpcHandler =
										new iframe_phone.IframePhoneRpcEndpoint(
											self.iframePhoneHandler,
											'data-interactive',
											codapIframe
										);
									var createCodapButton =
										document.createElement('button');
									createCodapButton.classList.add('btn');
									createCodapButton.classList.add(
										'btn-primary'
									);
									createCodapButton.innerHTML = 'Go!';
									createCodapButton.id =
										this.elementId + '_create_codap';
							...
							},
							{
								...
								value: function createSheet() {
									...
									wb.Props = {
										Title:
											'New Spreadsheet' + this.elementId,
										CreatedDate: new Date(),
									};
									wb.SheetNames.push(
										'Sheet' + this.elementId
									);
									var ws_data =
										this.csvParser
											.completeCsvMatrixTranspose;
							...
							{
								key: 'afterSampleData',
								value: function afterSampleData(
									flag,
									type
								) {
									var _this2 = this;
									...
									document.getElementById(
										...
										var ix = $(
											'input[name=' +
												_this2.tableXInputName +
												']:checked'
										).val();
										hash['x_axis_labels'] =
											_this2.csvParser.completeCsvMatrix[
												ix
											];
										...
										for (
											...
											hash['y_axis_values' + i] =
												_this2.csvParser.completeCsvMatrix[
													columns[i]
												];
											y_axis_names.push(
												_this2.csvParser.csvHeaders[
													columns[i]
												]
											);
										}
										...
										var labels = [
											_this2.csvParser.csvHeaders[ix],
											y_axis_names,
										];
										hash['labels'] = labels;
										var selectedGraph = $('.selected');
										var type =
											selectedGraph.attr('data-value');
							...
							},
							{
								...
								value: function graphMenu(flag) {
									...
									$('.radio').hover(
										function () {
											var tooltipVal = self.setTooltipGraph(
							...
							},
							{
								...
								value: function tableGenerator(
									...
									for (
										var i = 0;
										i < this.csvParser.csvHeaders.length;
										i++
									) {
									...
									for (
										var i = 0;
										i <
										this.csvParser.csvSampleData[0].length;
										i++
									) {
										...
										for (
											var j = 0;
											j < this.csvParser.csvHeaders.length;
											j++
										) {
											var td = document.createElement('td');
											td.appendChild(
												document.createTextNode(
													this.csvParser.csvSampleData[j][i]
												)
											);
											tr.appendChild(td);
										}
							...
							},
							{
								...
								value: function showSampleDataXandY() {
									...
									document.getElementById(
										this.addGraphButtonId
									).onclick = function (e) {
										_this3.graphCounting++;
										$('.' + _this3.carousalClass).carousel(
											1
										); /// ---------------> after

										_this3.tableGenerator(
											_this3.tableXInputName,
											_this3.tableXId,
											'radio',
											_this3.csvParser.csvHeaders,
											false,
											'table-success',
											'badge-success'
										);
										...
										_this3.tableGenerator(
											_this3.tableYInputName,
											_this3.tableYId,
											'checkbox',
											_this3.csvParser.csvValidForYAxis,
											false,
											'table-warning',
											'badge-warning'
										);
									...
									this.tableGenerator(
										this.tableXInputName,
										this.tableXId,
										'radio',
										this.csvParser.csvHeaders,
										true,
										'table-success',
										'badge-success'
									);
									this.tableGenerator(
										this.tableYInputName,
										this.tableYId,
										'checkbox',
										this.csvParser.csvValidForYAxis,
										true,
										'table-warning',
										'badge-warning'
									);
							...
							},
							{
								...
								value: function listenersForIntegration() {
									...
									$('#' + this.fileUploadId).change(
										function (e) {
											document.getElementById(
												'popover' + _this4.fileUploadId
											).style.display = 'inline';
											document.getElementById(
												'popover' + _this4.csvStringUploadId
											).style.display = 'none';
											document.getElementById(
												'popover' +
													_this4.googleSheetUploadId
											).style.display = 'none';
											document.getElementById(
												'popover' +
													_this4.remoteFileUploadId
											).style.display = 'none';

											_this4.createPopover(
												'popover' + _this4.fileUploadId
											);
									...
									$('#' + this.csvStringUploadId).change(
										function () {
											...
											_this4.handleFileSelectstring(
												document.getElementById(
													_this4.csvStringUploadId
												).value
											);
										}
									);
									...
									$('#' + this.remoteFileUploadId).change(
										function () {
											...
											_this4.sendRemoteFileToHandler(
												document.getElementById(
													_this4.remoteFileUploadId
												).value
											);
						...
						function View(elementId) {
							...
							this.remoteFileUploadId =
								elementId + '_remote_file';
							this.csvStringUploadId =
								elementId + '_csv_string';
							this.googleSheetUploadId =
								elementId + '_google_sheet';
							this.dragDropHeadingId =
								elementId + '_drag_drop_heading';
							this.uploadButtonId =
								elementId + '_file_upload_button';
							this.addGraphButtonId =
								elementId + '_add_graph';
							this.createSpreadsheetButtonId =
							...
							this.tableXInputName =
								elementId + '_x_axis_input_columns';
							this.tableYInputName =
								elementId + '_y_axis_input_columns';
							...
							this.graphMenuTypeInputName =
								elementId + '_types';
							this.canvasContinerId =
								elementId + '_canvas_container';
							this.xyToggleName = elementId + '_xytoggle';
							this.saveAsImageId =
								elementId + 'save-as-image';
							this.upload_button_container =
								elementId + 'upload_button_container';
							this.feature_button_container =
								elementId + 'feature_button_container';
							this.codapExportButton =
								elementId + 'codap_export_button';
							this.drawHTMLView();
							this.addListeners();
							var self = this;
							...
							$(
								...
								$('#' + _this5.tableXParentId).toggle(
									ixx === 0
								);
								$('#' + _this5.tableYParentId).toggle(
									ixx === 1
								);
							});
							$('.imports').hover(
								function () {
									var tooltipVal = self.setTooltip(
										this.classList[0]
									);
						...
						_createClass(View, [
							{
								...
								value: function addListeners() {
									...
									$('#' + this.csvStringUploadId).change(
										function () {
											_this6.handleFileSelectstring(
												document.getElementById(
													_this6.csvStringUploadId
												).value
											);
										}
									);
									...
									$('#' + this.remoteFileUploadId).change(
										function () {
											_this6.sendRemoteFileToHandler(
												document.getElementById(
													_this6.remoteFileUploadId
												).value
											);
										}
									);
							...
							},
							{
								...
								value: function drawHTMLView() {
									this.element.innerHTML =
										...
										' class="carousel ' +
										this.carousalClass +
										' slide" data-ride="carousel" data-interval="false"> <div class="indicators"> <ol class="carousel-indicators"> <li data-target="#' +
										this.carousalId +
										'" data-slide-to="0" class="active" id="up" class="first_indicator"></li> <li data-target="#' +
										this.carousalId +
										'" data-slide-to="1" class="second_indicator"></li> <li data-target="#' +
										this.carousalId +
										'" data-slide-to="2" class="third_indicator"></li> </ol> </div> <div class="carousel-inner"> <div class="carousel-item active"> <div class="parent_main_container"> <div> <p id="import_description"> text_for_replacement</p> </div> <div class="main_container"> <div class="main_grid_container"> <div class="container_drag_drop grid-item imports"> <p class="sub_heading_import"> Local File </p> <span class="btn btn-outline-primary btn-file input_box shadow"> <p class="drag_drop_heading" id=' +
										this.dragDropHeadingId +
										'><u> Choose a csv file </u> or drag & drop it here </p> <input type="file" class="csv_file" id=' +
										this.fileUploadId +
										' accept=".csv"> </span> <button type="button" class="btn btn-dark des" id=' +
										'popover' +
										this.fileUploadId +
										'> <i class="fa fa-list"></i> </button> </div> <div class="container_remote_link grid-item imports"> <p class="sub_heading_import"> Remote File </p> <input type="text" class="remote_file text_field shadow" placeholder="url of remote file" id=' +
										this.remoteFileUploadId +
										'> <button type="button" class="btn btn-dark des" id=' +
										'popover' +
										this.remoteFileUploadId +
										'><i class="fa fa-list"></i></button> </div> <div class="container_csv_string grid-item imports"> <p class="sub_heading_import"> String File </p> <textarea class="csv_string text_field shadow" id=' +
										this.csvStringUploadId +
										' placeholder="Paste a CSV string here"></textarea> <button type="button" class="btn btn-dark des" id=' +
										'popover' +
										this.csvStringUploadId +
										'><i class="fa fa-list"></i></button> </div> <div class="container_google_sheet grid-item imports"> <p class="sub_heading_import"> Google Sheet </p> <div class="google_sheet_container"> <input type="text" class="google_sheet text_field shadow" id=' +
										this.googleSheetUploadId +
										' placeholder="Link of published Google Sheet"> <button type="button" class="btn btn-dark des" id=' +
										'popover' +
										this.googleSheetUploadId +
										'><i class="fa fa-list"></i></button> </div> </div> </div> <div id=' +
										this.upload_button_container +
										' class="upload_button"> <button type="button" class="btn btn-primary uploadButton" id=' +
										this.uploadButtonId +
										'>Upload CSV</button> </div> </div> <div style="visibility: hidden;"> <p> heyyyyy</p> </div> </div> </div> <div class="carousel-item tables"> <div class="button_container"> <div> <input type="checkbox" name=' +
										this.xyToggleName +
										' checked data-toggle="toggle" class="xytoggle" id="xy" data-width="150" data-onstyle="success" data-offstyle="warning" data-height="40"> </div> <div class="plot_button"> <button type="button" class="btn btn-primary plotGraph" id=' +
										this.plotGraphId +
										'>Plot Graph</button> </div> </div> <div class="table_container"> <div id=' +
										this.tableXParentId +
										'> <table id=' +
										this.tableXId +
										' class="table"></table> </div> <div id=' +
										this.tableYParentId +
										' class="hidden"> <table id=' +
										this.tableYId +
										' class="table"></table> </div> <div id="menu_holder"></div> </div> </div> <div class="carousel-item graph"> <div id=' +
										this.feature_button_container +
										' class="feature_buttons"> <button type="button" class="btn btn-primary addGraph" id=' +
										this.addGraphButtonId +
										'> Add Graph</button> <button type="button" class="btn btn-success createSpreadsheet" id=' +
										this.createSpreadsheetButtonId +
										'> Create Spreadsheet &nbsp <i class="fa fa-plus" aria-hidden="true"></i></button> <button type="button" class="btn btn-info codapExport" id=' +
										this.codapExportButton +
										' data-toggle="modal" data-target="#exampleModalCenter">View and Export to CODAP</button> </div> <div class="parent_canvas_container"> <div id=' +
										this.canvasContinerId +
										'></div> </div> </div> </div> </div></div><div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true"> <div class="modal-dialog modal-lg modal-dialog-centered" id="modal-style" role="document"> <div class="modal-content"> <div class="modal-header"> <h5 class="modal-title" id="exampleModalLongTitle">CODAP</h5> <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span> </button> </div> <div class="modal-body" id="body_for_CODAP"></div> </div> </div></div>';
								},
							},
						]);
  ...
  */
					...
					function startPostingHello() {
						...
						helloInterval = window.setInterval(
							postHello,
							HELLO_INTERVAL_LENGTH
						);
						window.setTimeout(
							stopPostingHello,
							HELLO_TIMEOUT_LENGTH
						);
		...
		7: [
			function (require, module, exports) {
				...
				function getPseudoUUID() {
					...
					for (var i = 0; i < 10; i++) {
						ret.push(
							chars[Math.floor(Math.random() * len)]
						);
					}
					return ret.join('');
				}

				module.exports = function IframePhoneRpcEndpoint(
					handler,
					namespace,
					targetWindow,
					targetOrigin,
					...
					phone.addListener(
						...
						function (message) {
							...
							if (
								...
								message.messageType === 'returnValue'
							) {
								callbackObj =
									pendingCallbacks[message.uuid];
							...
							timeout: window.setTimeout(function () {
								if (callback) {
									callback(
										undefined,
										new Error(
											'IframePhone timed out waiting for reply'
										)
									);
								}
							}, 2000),
			...
			{ './iframe-endpoint': 6, './parent-endpoint': 8 },
		],
		8: [
			function (require, module, exports) {
				var structuredClone = require('./structured-clone');
...
*/
				...
				module.exports = function ParentEndpoint(
					targetWindowOrIframeEl,
					targetOrigin,
					...
					function getTargetWindow() {
						if (targetWindowIsIframeElement) {
							var tWindow =
								targetWindowOrIframeEl.contentWindow;
							if (!tWindow) {
								throw (
									'IFrame element needs to be added to DOM before communication ' +
					...
					function receiveMessage(message) {
						var messageData;
						if (
							message.source === getTargetWindow() &&
							(targetOrigin === '*' ||
							...
							if (handlers[messageData.type]) {
								...
								console.log(
									'cant handle type: ' + messageData.type
								);
							}
						}
					}
					...
					try {
						targetWindowIsIframeElement =
							targetWindowOrIframeEl.constructor ===
							HTMLIFrameElement;
					} catch (e) {
					...
					if (targetWindowIsIframeElement) {
						...
						if (
							!targetOrigin ||
							targetOrigin.constructor === Function
						) {
					...
					addListener('hello', function () {
						...
						if (
							afterConnectedCallback &&
							typeof afterConnectedCallback === 'function'
						) {
							afterConnectedCallback();
						}
		...
		9: [
			function (require, module, exports) {
				...
				exports.supported = function supported() {
					return (
						featureSupported &&
						featureSupported.structuredClones > 0
					);
				};
			},
			{},
		...
		10: [
			...
			{
				'./lib/iframe-endpoint': 6,
				'./lib/iframe-phone-rpc-endpoint': 7,
				'./lib/parent-endpoint': 8,
				'./lib/structured-clone': 9,
			},
		],
		11: [
			function (require, module, exports) {
				...
				Array.isArray ||
					...
					(function (e, t) {
						...
						var s,
							...
							k = {
								parse: function (e, t) {
									...
									z(r) &&
										((t.dynamicTypingFunction = r),
										(r = {}));
									if (
										((t.dynamicTyping = r),
										(t.transform =
											!!z(t.transform) && t.transform),
									...
									if (Array.isArray(e)) {
										...
										return (
											...
											Array.isArray(e.data) &&
												(e.fields ||
													(e.fields =
														e.meta && e.meta.fields),
												e.fields ||
													(e.fields = Array.isArray(
														e.data[0]
													)
														? e.fields
														: a(e.data[0])),
							...
							d.fn.parse = function (o) {
								...
								function e() {
									if (0 !== h.length) {
										...
										(n.instanceConfig.complete = function (
											e
										) {
											z(a) && a(e, n.file, n.inputElem),
												u();
										}),
							...
							(this._handle = null),
								...
								(this.parseChunk = function (e, t) {
									if (
										...
										var r =
											this._config.beforeFirstChunk(e);
										void 0 !== r && (e = r);
									}
									this.isFirstChunk = !1;
									...
									if (
										...
										var s = n.meta.cursor;
										this._finished ||
											((this._partialLine = i.substring(
												s - this._baseIndex
											)),
											(this._baseIndex = s)),
											n &&
												n.data &&
												(this._rowCount += n.data.length);
										var a =
											this._finished ||
						...
						function p(e) {
							var i, n;
							(e = e || {}).chunkSize ||
								(e.chunkSize = k.LocalChunkSize),
								l.call(this, e);
							var s = 'undefined' != typeof FileReader;
							(this.stream = function (e) {
								(this._input = e),
									(n =
										e.slice || e.webkitSlice || e.mozSlice),
									s
										? (((i = new FileReader()).onload = w(
												this._chunkLoaded,
												this
										  )),
										  (i.onerror = w(
							...
							function _(e, t) {
								return (
									...
									g.dynamicTypingFunction &&
										...
										(g.dynamicTyping[r] =
											g.dynamicTypingFunction(r)),
									!0 ===
									(g.dynamicTyping[r] || g.dynamicTyping)
										? 'true' === t ||
										  'TRUE' === t ||
										  ('false' !== t &&
												'FALSE' !== t &&
							...
							(this.parse = function (e, t, r) {
								...
								if (
									...
									n.successful
										...
										: ((h = !0),
										  (g.delimiter = k.DefaultDelimiter)),
										(d.meta.delimiter = g.delimiter);
								}
								var s = E(g);
								return (
								...
								for (
									...
									if (i[P] !== S)
										if (
											...
											f.push(i.substring(P, _)),
												(P = _ + e),
												(_ = i.indexOf(O, P));
										else {
											if (-1 === g) break;
											if (
												(f.push(i.substring(P, g)),
												w(g + s),
												o && (R(), F))
											)
												return C();
						...
						function m(e) {
							...
							if (t.error) r.userError(t.error, t.file);
								...
								if (z(r.userStep)) {
									...
									z(r.userChunk) &&
										(r.userChunk(t.results, n, t.file),
										delete t.results);
							}
						...
						function y(e, t) {
							var r = h[e];
							z(r.userComplete) && r.userComplete(t),
								r.terminate(),
								delete h[e];
						}
						function b() {
